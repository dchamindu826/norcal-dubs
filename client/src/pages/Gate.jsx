    ...
    <div className="relative w-full h-screen flex flex-col items-center justify-center bg-black overflow-hidden">
      ...
      <div className="z-10 flex flex-col items-center w-full px-4">
        ...
        <div className="text-center mb-12">
            ...
            <p className="text-gray-500 text-[10px] md:text-xs tracking-[0.8em] uppercase mt-4 opacity-80">Premium Access Only</p>
        </div>
        <form onSubmit={handleLogin} className="w-full max-w-xs relative">
          <div className="relative group">
            <Lock className="absolute left-6 top-1/2 transform -translate-y-1/2 text-gray-500 w-5 h-5 group-focus-within:text-[#39FF14] z-20" />
            <input type="password" placeholder="ACCESS CODE" className={`w-full bg-black/40 backdrop-blur-md border border-white/10 ${error ? 'border-red-500 shake-animation' : 'focus:border-[#39FF14]'} rounded-full px-12 py-4 text-white text-center tracking-[0.3em] font-light outline-none transition-all placeholder:text-gray-700`} value={input} onChange={(e) => setInput(e.target.value)} autoFocus />
            <button type="submit" className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-[#39FF14] p-2"><ArrowRight size={20} /></button>
          </div>
