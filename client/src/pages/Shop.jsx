...
const Shop = () => {
  ...
  const [activeMedia, setActiveMedia] = useState(null); // Holds URL of current view

  useEffect(() => {
    const allProducts = getProducts();
    setProducts(allProducts.reverse());
    setFilteredProducts(allProducts);
    const allCats = ['All', ...getCategories()];
  ...
  const openProduct = (item) => {
    ...
    // Set default media to first image (or first video if no images)
            ...
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold uppercase tracking-wider transition-all whitespace-nowrap ${
                activeCategory === cat 
                  ...
                  : 'bg-[#111] text-gray-400 border border-white/10 hover:border-white'
              }`}
            >
              {cat}
            </button>
        ...
        <AnimatePresence>
          {filteredProducts.map((item) => (
            <motion.div 
              ...
              className="bg-[#0a0a0a] rounded-2xl overflow-hidden border border-white/5 hover:border-[#39FF14] group cursor-pointer relative shadow-lg transition-all duration-300"
            >
              {item.specialOffer && <div className="absolute top-3 left-3 z-10 bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg animate-pulse">SPECIAL OFFER</div>}
                  ...
                  <div className="flex flex-col">
                      {item.specialOffer ? (
                          <>
                              ...
                              <span className="text-[#39FF14] font-black text-2xl">${item.offerPrice}</span>
                          </>
                      ) : (
                          <span className="text-white font-black text-2xl">${item.price}</span>
                      )}
                  </div>
                  <button className="bg-white text-black p-2 rounded-full hover:bg-[#39FF14] transition-colors">
                    <ShoppingBag size={20} />
                  </button>
                </div>
              </div>
        ...
        {selectedProduct && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/95 backdrop-blur-lg">
            <motion.div 
              ...
              exit={{ opacity: 0, y: 50 }}
              className="bg-[#0a0a0a] w-full max-w-5xl max-h-[95vh] overflow-y-auto rounded-3xl border border-white/10 relative shadow-2xl flex flex-col md:flex-row"
            >
              <button 
                onClick={() => setSelectedProduct(null)}
                className="absolute top-4 right-4 z-20 bg-black/50 p-2 rounded-full text-white hover:bg-white hover:text-black transition-colors border border-white/10"
              >
                <X size={24} />
              </button>

              {/* 1. MEDIA GALLERY SECTION (Left Side) */}
              <div className="w-full md:w-1/2 bg-black flex flex-col border-b md:border-b-0 md:border-r border-white/10">
                
                {/* Main Viewport */}
                <div className="flex-1 relative flex items-center justify-center bg-[#050505] p-4 min-h-[300px] md:min-h-[400px]">
                  {activeMedia?.type === 'video' ? (
                    ...
                    <img 
                      src={activeMedia?.url || '/logo-nobg.png'} 
                ...
                <div className="h-24 bg-[#111] border-t border-white/10 flex items-center px-4 gap-3 overflow-x-auto scrollbar-hide">
                  ...
                  {selectedProduct.images.map((img, i) => (
                    <button 
                      ...
                      className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 flex-shrink-0 transition-all ${activeMedia?.url === img ? 'border-[#39FF14] opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}
                    >
                  ...
                  {selectedProduct.videos.map((vid, i) => (
                    <button 
                      ...
                      className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 flex-shrink-0 bg-gray-900 transition-all ${activeMedia?.url === vid ? 'border-[#39FF14] opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}
                    >
                  ...
                  <div className="mb-auto">
                    ...
                    <div className="mb-8 p-4 bg-white/5 rounded-2xl border border-white/10 inline-block">
                        {selectedProduct.specialOffer ? (
                             <div className="flex flex-col">
                                <span className="text-sm text-gray-500 line-through font-bold mb-1">REGULAR: ${selectedProduct.price}</span>
                                <div className="flex items-center gap-3">
                                  <span className="text-4xl font-black text-[#39FF14]">${selectedProduct.offerPrice}</span>
                                  <span className="bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded uppercase">Save Big</span>
                                </div>
                             </div>
                        ) : (
                    ...
                    <p className="text-gray-300 leading-relaxed text-sm md:text-base mb-8 border-l-2 border-[#39FF14] pl-4">
                        {selectedProduct.description || "No description available for this premium strain."}
                    </p>
                  </div>

                  <div className="mt-8 space-y-4">
                      <button className="w-full bg-[#39FF14] text-black font-black py-4 rounded-xl hover:shadow-[0_0_20px_rgba(57,255,20,0.4)] transition-all flex items-center justify-center gap-3 text-lg">
                          <Phone size={20} /> CALL TO ORDER
                      </button>
                      <a href="https://t.me/NorCalBudz707" target="_blank" rel="noreferrer" className="w-full bg-[#0088cc]/20 border border-[#0088cc]/50 text-[#0088cc] font-bold py-4 rounded-xl hover:bg-[#0088cc] hover:text-white transition-all flex items-center justify-center gap-3">
                          <Send size={20} /> ORDER VIA TELEGRAM
                      </a>
                  </div>
              </div>
